...
func New(config *Config, logging *zap.Logger, tracer trace.Tracer) (App, error) {
	...
	userRepo, err := userrepo.New(&config.Auth, pgxPool, logging, tracer)
	if err != nil {
		return nil, err
	}

	authService := authsvc.New(&config.Auth, userRepo, logging, tracer)

	a := &app{
		config:      config,
		authSerice:  authService,
		httpAdapter: httpadapter.New(&config.HTTP, authService, logging, tracer),
	}
