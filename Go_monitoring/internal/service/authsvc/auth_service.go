...
func (s *authService) newTokenPair(login string) (*model.TokenPair, error) {
	...
	if err != nil {
		s.logger.Error("Internal: can't make access token", zap.Error(err))
	...
	if err != nil {
		s.logger.Error("Internal: can't make refresh token", zap.Error(err))
		return nil, err
	}
...
func (s *authService) Login(ctx context.Context, login, password string) (*model.TokenPair, error) {
	newCtx, span := s.trace.Start(ctx, "service: login")
	defer span.End()
	ctx = zapctx.WithLogger(newCtx, s.logger)
...
func (s *authService) ValidateAndRefresh(ctx context.Context, tokenPair *model.TokenPair) (new *model.TokenPair, err error) {
	newCtx, span := s.trace.Start(ctx, "service: validateAndRefresh")
	defer span.End()
	ctx = zapctx.WithLogger(newCtx, s.logger)

	span.AddEvent("parsing JWT")
...
func New(config *service.AuthConfig, repo repo.User, logging *zap.Logger, tracer trace.Tracer) service.Auth {
	return &authService{
		repo:                 repo,
